...
export const config: Options.Testrunner = {
    ...
    afterStep: async function (step, scenario, result) {
        if(!result.passed) {
            ...
            const filePath = path.resolve(screenshotDir,`${scenario.name}-${timestamp}.png`);
            await browser.saveScreenshot(filePath); // screenshot saved in the folder
            console.log(`Failed Step:${step}, screenshot taken and stored in the path-${filePath}`);   
                     
            // // screenshot to be attached in the cucumber-html-report
